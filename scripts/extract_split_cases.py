...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--output-root",
        help="Directory where outputs will be written (defaults to data/curated/<book_id>/extracted_cases)."
    )
    parser.add_argument(
        "--backend",
        choices=["ocrmypdf", "native"],
        default="native",
        help="Extraction backend to use (default: native)."
    )
    parser.add_argument(
        "--section",
...
def main() -> int:
    ...
    output_root = Path(args.output_root) if args.output_root else (Path("data") / "curated" / book_id / "extracted_cases")
    ...
    for c in cases:
        ...
        try:
            ...
            # 2. Run Extraction Backend
            if args.backend == "native":
                from scanbook.ocr.native_pdf_backend import NativePdfBackend
                backend = NativePdfBackend()
                print("  Running Native PyMuPDF backend...")
            else:
                from scanbook.ocr.ocrmypdf_backend import OcrmypdfBackend
